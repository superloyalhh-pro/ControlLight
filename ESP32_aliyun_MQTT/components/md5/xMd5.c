					   ...
					   uint8_t *outBuffer)
{
	...
	uint8_t output[typeMd5], dst[40];
	uint8_t *q = (uint8_t *)strContent;
	XH_MD5_CTX XH_MD5;
	XH_MD5Init(&XH_MD5);
	...
	{
		sprintf((char *)outBuffer, "%02x%02x%02x%02x%02x%02x%02x%02x", output[4],
...
void HMAC_XH_MD5(uint8_t *inBuffer, uint16_t len, uint8_t *ky, char *pUserOut)
{
	...
	if (strlen((char *)ky) > 64)
	{
		XH_MD5Digest(ky, strlen((char *)ky), key);
	}
	else
	{
		STRNCPY((char *)key, (char *)ky, 64);
	}
